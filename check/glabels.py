...
def kifloat(float_num):
    return ('%.2f'%float_num).rstrip('0').rstrip('.')
    ...
    for ii in chkseq:
        ...
        for jj in range(1, len(ii), 2):
            ...
            if ii[jj-1].getPosPage() == ii[jj].getPosPage():
                ...
                frst.setFieldPos('Intersheetrefs', kifloat(x), kifloat(y), str(orient))
                sec.setField('Intersheetrefs', '/'+frst.getPositionHuman())
                x, y, orient = calcXYref(sec)
                sec.setFieldPos('Intersheetrefs', kifloat(x), kifloat(y), str(orient))
